...
When spawning each subagent, classify the task's difficulty and assign the model per the **Task Routing — Model per Difficulty** section below. Do not run every task on the largest model.
...
## Task Routing — Model per Difficulty

When the orchestrator (Step 6) spawns subagents, it MUST classify each task and pick the model accordingly (via the Agent tool's `model` parameter). Route by difficulty, not by habit — easy tasks must NOT consume the largest model.

### Easy → `haiku`
Mechanical, low-risk, well-specified tasks with no design decisions:
- Renaming variables/functions, fixing typos, updating comments or docstrings
- Formatting, import cleanup, small config/`.env.example` edits
- Simple documentation updates (README, CLAUDE.md)
- Adding a straightforward field to an existing schema/model following an established pattern
- Writing a simple unit test that mirrors an existing test's structure

### Medium → `sonnet`
Localized changes requiring some judgment but confined to one layer or file cluster:
- Adding a new API endpoint that follows existing endpoint patterns (e.g., in `sams_backend/api/`)
- Bug fixes with a known root cause
- Extending an existing service function or WebSocket message type
- Writing test suites for a single module
- Frontend changes to `sams_dashboard/index.html` following existing UI patterns

### Complex → `opus` (or inherit the session model)
Cross-cutting, security-sensitive, or design-heavy tasks:
- Anything touching authentication, authorization, encryption, or input validation at trust boundaries (Step 7 areas)
- Changes spanning backend + frontend (e.g., new end-to-end feature)
- Modifying the processing pipeline (`sams_backend/services/processing_pipeline.py`) or NLP/alerting logic
- Database schema migrations or changes to `models/database.py`
- Debugging with an unknown root cause; performance or concurrency work

### Rules
- **The orchestrator itself** always runs on the session's default model — it plans and reviews, never codes.
- **When in doubt, route one tier up.** Misrouting a complex task to `haiku` costs more in rework than it saves.
- **Escalate on failure:** if a subagent's result fails review or tests, re-dispatch the task one model tier higher rather than retrying on the same tier.
- **Read-only exploration/search** tasks (finding files, summarizing code) default to `haiku` regardless of the parent task's difficulty.
