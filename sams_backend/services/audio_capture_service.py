...
services/audio_capture_service.py
═══════════════════════════════════════════════════════
MODULE 1: Speech Detection & Audio Capture (Supabase Stream Edition)
═══════════════════════════════════════════════════════
"""
import io
import logging
...
class AudioCaptureService:
    ...
    def __init__(
        ...
        max_duration_secs: float = 12.0
    ):
        self.bucket_name = bucket_name
        # Initialize Supabase client inside the service layer
        self.supabase_client: Client = create_client(supabase_url, supabase_key)
        self.vad_threshold = vad_threshold
        self.max_duration_secs = max_duration_secs

    def process_incoming_clip(self, raw_bytes: bytes, filename: str, event_id: str) -> Tuple[str, float]:
        """
        Flow Step 2: Processes incoming audio array, validates it, and uploads 
        it straight to your designated Supabase Storage Bucket.
        Returns: (supabase_file_path, duration_seconds)
        """
        logger.info(f"Processing raw edge event clip context payload ({len(raw_bytes):,} bytes)")

        try:
            with io.BytesIO(raw_bytes) as bio:
                data, samplerate = sf.read(bio)
        except Exception as e:
            raise ValueError(f"Invalid audio encoding formats or header corrupted: {e}")

        # Calculate actual recorded window length duration
        num_samples = data.shape[0]
        duration = num_samples / samplerate
        logger.info(f"Ingested sound track file specs: {samplerate}Hz, {duration:.2f}s duration context")

        # Preprocess mapping: Mono tracking & downsampling execution
        processed_data = self._preprocess(data, samplerate)
        
        # Run local baseline VAD validation filtering matrix check
        has_speech = self._validate_vad(processed_data)
        if not has_speech:
            logger.warning("Voice activity validation tracking returned empty noise threshold parameters")

        # ── Write formatted byte stream directly to In-Memory buffer ─────────
        out_bio = io.BytesIO()
        sf.write(out_bio, processed_data, TARGET_SAMPLE_RATE, format='WAV', subtype='PCM_16')
        ...
        try:
            ...
            logger.error(f"Supabase target cluster stream storage injection failed: {e}")
            raise RuntimeError(f"Cloud instance bucket storage crash mapping context: {str(e)}")

        return supabase_file_path, duration

    def _validate_vad(self, audio_array: np.ndarray) -> bool:
        """Simple Voice Activity Detection based on segment energy thresholds."""
        frame_len = int(0.030 * TARGET_SAMPLE_RATE)  # 30ms windows
        ...
        active_ratio = active_frames / num_frames if num_frames > 0 else 0
        return active_ratio > 0.20  # True if more than 20% contains significant sound

    def _preprocess(self, audio_array: np.ndarray, sample_rate: int) -> np.ndarray:
        """Resample to 16 kHz mono and normalize amplitude bounds."""
        import librosa

        # Ensure layout conforms explicitly to a single mono tracking array layout
        mono = audio_array if audio_array.ndim == 1 else audio_array.mean(axis=0)
        ...
        max_samples = int(self.max_duration_secs * TARGET_SAMPLE_RATE)
        return mono[:max_samples].astype(np.float32)